...
# Try to import pyttsx3, install if not available
try:
    import pyttsx3
except ImportError:
    print("pyttsx3 not found. Installing it now...")
    subprocess.check_call([sys.executable, "-m", "pip", "install", "pyttsx3"])
    import pyttsx3  # Try importing again after installation

# Function to make voice announcements
def announce(msg):
    """
    Make a voice announcement using pyttsx3.
    
    Args:
        msg (str): Message to be announced
    """
    try:
        engine = pyttsx3.init()
        engine.say(msg)
        ...
        # Continue with the program even if voice fails
...
def remove_candidate():
    """
    Remove a candidate from the election system.
...
def remove_voter():
    """
    Remove a voter from the election system.
...
def display_welcome_message():
    ...
    Also makes a voice announcement.
    """
    welcome_message = "Welcome to the Ericsson Mediation Election!"
    print(f"\n🎉 {welcome_message} 🎉\nCandidates standing for election are:\n")
    
    # Announce the welcome message
    announce(welcome_message)
    
    # Build candidates message for announcement
    candidates_list = list(candidates.keys())
    candidates_message = "Candidates standing for election are: " + ", ".join([c.title() for c in candidates_list])
    ...
    # Announce candidates
    announce(candidates_message)
    announce("Let's begin voting! Each voter can vote only once.")
...
def process_vote(voter_name):
    ...
    """
    # Authentication step
    announce(f"Hello {voter_name}. Please enter your SIGNUM for authentication.")
    signum_credential = input("Please enter your SIGNUM for authentication:\n").lower()

    if authenticate_voter(voter_name, signum_credential):
        auth_message = "Authentication successful! Proceed to vote."
        print(f"\n✅ {auth_message}")
        announce(auth_message)
        
        display_candidates()
        announce("Please select your candidate from the list.")
        ...
        if is_valid_candidate(candidate_choice):
            success_message = f"Congratulations! Your vote has been cast for {candidate_choice.title()}"
            print(f"\n🎉 {success_message}")
            ...
            if not eligible_voters:
                all_voted_message = "All voters have completed their voting."
                print(f"🎯 {all_voted_message}")
                announce(all_voted_message)
            else:
                next_voter_message = "Please allow the next voter to proceed."
                print(f"\n➡️ {next_voter_message}\n")
                announce(next_voter_message)
            return True
        else:
            fail_message = "Voting failed: Candidate not recognized."
            print(f"❌ {fail_message}")
            announce(fail_message)
            return False
    else:
        auth_fail_message = "Authentication failed: SIGNUM mismatch. Vote not recorded."
        print(f"🚫 {auth_fail_message}")
        announce(auth_fail_message)
        return False
...
def run_election():
    ...
    if not voter_credentials:
        message = "There are no registered voters. Please add voters before starting the election."
        print(f"⚠️ {message}")
        announce(message)
        return
        
    if not candidates:
        message = "There are no registered candidates. Please add candidates before starting the election."
        print(f"⚠️ {message}")
        announce(message)
        return
    
    # Announce that voting is now open
    announce("Voting is now open. Please enter your name to cast your vote.")
...
def end_election():
    ...
    """
    end_message = "Voting session ended. Let's look at the results."
    print(f"\n🔚 {end_message}\n")
    announce(end_message)
    
    # Show the final results
    display_results()
    
    # Announce results
    results_message = "Final results: "
    for candidate, votes in candidates.items():
        results_message += f"{candidate.title()}: {votes} votes. "
    announce(results_message)
    ...
    winning_candidates, highest_vote_count = find_winners()
    if len(winning_candidates) == 1:
        winner_message = f"The winner is {winning_candidates[0].title()} with {highest_vote_count} votes!"
        announce(winner_message)
    else:
        tie_message = f"It's a tie between {', '.join([c.title() for c in winning_candidates])} with {highest_vote_count} votes each."
        announce(tie_message)
...
def menu_system():
    ...
    """
    welcome_message = "Welcome to the Ericsson Mediation Election System"
    print(f"\n🗳️ {welcome_message} 🗳️")
    announce(welcome_message)
    ...
    while True:
        ...
        if choice == "1":
            # Start the election
            announce("Starting the election process")
            run_election()
        elif choice == "2":
            # Enter admin menu
            announce("Opening admin menu. Authentication required.")
            admin_menu_system()
        elif choice == "3":
            # View registered voters
            announce("Displaying registered voters")
            view_registered_voters()
        elif choice == "4":
            # View candidates
            announce("Displaying registered candidates")
            view_candidates()
        elif choice == "5":
            # View current election results (NEW OPTION)
            announce("Displaying current election results")
            view_current_results()
        elif choice == "6":
            # Exit the system
            goodbye_message = "Thank you for using the Election System. Goodbye!"
            print(f"\n👋 {goodbye_message}")
            announce(goodbye_message)
            break
        else:
            error_message = "Invalid choice. Please select a number between 1 and 6."
            print(f"\n⚠️ {error_message}")
